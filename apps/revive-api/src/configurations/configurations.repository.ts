...
export class MongoConfigurationsRepository {
  ...
  async create(configuration: SavedConfigurationRecord) {
    const db = await getMongoDb()
    await db.collection<SavedConfigurationRecord>(CONFIGURATIONS_COLLECTION).insertOne(configuration)
  ...
  async deleteById(id: string) {
    const db = await getMongoDb()
    const result = await db.collection<SavedConfigurationRecord>(CONFIGURATIONS_COLLECTION).deleteOne({ id })
