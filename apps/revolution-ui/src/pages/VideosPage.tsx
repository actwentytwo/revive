...
import type { MigrationProject, SourceVideoRecord } from "@revolution/shared";
...
export function VideosPage({
  ...
  return (
    <Stack spacing={3}>
      <PageIntro
        title="Videos"
        subtitle={`Browse the source library attached to ${activeProject?.name ?? "the selected project"}.`}
      />

      {!connected ? (
        <Alert severity="warning">
          Validate the selected source configuration before browsing videos.
        </Alert>
      ) : null}
      ...
      <Paper className="panel panel-full">
        <PanelTitle
          ...
          subtitle="Source videos from the selected platform configuration."
        />
        <Stack spacing={2}>
          <TextField
