...
import type { MigrationProject } from '@revive/shared'
...
export function AppShellHeader({
  ...
  return (
    <Paper className="topbar">
      ...
      >
        <Stack component={Link} to={homePath} direction="row" spacing={1.5} alignItems="center" className="brand-link">
          <ReviveLogo />
          <Box>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', md: 'center' }}>
          ...
          <FormControl size="small" className="project-select">
            <InputLabel id="project-select-label" shrink={Boolean(activeProjectId)}>
              {Boolean(activeProjectId) ? 'Project' : projects.length === 0 ? 'No Projects' : 'Select Project' }
            </InputLabel>
            <Select
              ...
              renderValue={(value) => projects.find((project) => project.slug === value || project.id === value)?.name ?? ''}
            >
              {projects.map((project) => (
                <MenuItem key={project.id} value={project.slug} onClick={() => onSelectProject(project.slug)}>
                  {project.name}
                </MenuItem>
              ))}
            </Select>
          ...
          <Tooltip title="Create new project">
            <span>
              <IconButton className="theme-toggle" color="inherit" disabled={apiOffline} onClick={onCreateProject}>
                <AddRounded />
              </IconButton>
            </span>
          </Tooltip>
