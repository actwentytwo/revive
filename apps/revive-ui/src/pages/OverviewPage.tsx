...
import type { MigrationProject, SavedConfiguration } from '@revive/shared'
import { Fact, MetricPill, PageIntro, PanelTitle, formatProjectType, formatTimestamp } from '../components/shared/PageChrome'
    ...
    <Stack spacing={3}>
      ...
      <section className="content-grid">
        <Paper className="panel">
          <PanelTitle icon={<CloudDoneRounded />} title="Project Status" subtitle="A summary of the current migration project." />
          <Stack direction="row" spacing={2} useFlexGap flexWrap="wrap">
            <MetricPill label="Project" value={activeProject?.name ?? 'None'} />
            <MetricPill label="Type" value={activeProject ? formatProjectType(activeProject.projectType) : 'None'} />
            <MetricPill label="Source" value={sourceConfiguration?.name ?? 'Not selected'} />
            <MetricPill label="Destination" value={destinationConfiguration?.name ?? 'Not selected'} />
            <MetricPill label="Videos loaded" value={String(rowCount)} />
          </Stack>
        </Paper>

        <Paper className="panel">
          <PanelTitle icon={<DashboardRounded />} title="Project Details" subtitle="Core information for the selected project." />
          {activeProject ? (
            <Stack spacing={2}>
              ...
              <Fact label="Source configuration" value={sourceConfiguration?.name ?? 'Not selected'} />
              <Fact label="Destination configuration" value={destinationConfiguration?.name ?? 'Not selected'} />
              <Fact
                label="Readiness"
                value={
