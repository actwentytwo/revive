          ...
          {projects.length === 0 ? (
            <Box className="empty-state">
              ...
              <Typography color="text.secondary" className="empty-state-copy">
                Projects are where you define a migration or any future workflow. Start by creating one, then add
                configuration when you’re ready.
              </Typography>
              <Button variant="contained" size="large" startIcon={<AddRounded />} onClick={onOpenCreateProjectDialog}>
                Create Project
              </Button>
            </Box>
          ) : (
            projects.map((project) => {
              const sourceConfiguration = configurations.find((item) => item.id === project.sourceConfigurationId)
              const destinationConfiguration = configurations.find((item) => item.id === project.destinationConfigurationId)
              const configured =
